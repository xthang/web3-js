...
import { AddressLike, BigNumberish, ChainNamespace, FetchRequest, Listener, Network, Networkish, convertToHexAddress, makeError } from "../ethers";
...
export class TronProvider implements Provider {
    ...
    constructor(rpcUrl: string | FetchRequest, fullHostUrl: string | providers.HttpProvider, network?: Networkish) {
        this.jsonRpcProvider = new TronJsonRpcProvider(rpcUrl, network)
        this.tronWeb = new TronWeb({
            fullHost: fullHostUrl,
            headers: { "TRON-PRO-API-KEY": 'a4f709c9-c7a8-4ddc-ab7f-f576bea96a9b' },
    ...
    async getBalance(address: AddressLike, blockTag?: BlockTag): Promise<bigint> {
        ...
        return this.jsonRpcProvider.getBalance(convertToHexAddress(address, this.chainNamespace), blockTag)
    }
    ...
    async broadcastTransaction(signedTx: { txID: string }): Promise<TransactionResponse> {
        ...
        if ('message' in resp && resp.message)
            resp.message = `[decoded] ${TronWeb.toUtf8(resp.message)}`
