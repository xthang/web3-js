...
import { convertToHexAddress, resolveAddress } from "../address";
...
export class TronJsonRpcProvider extends JsonRpcProvider {
   ...
   _getTransactionRequest(_request: TransactionRequest): PerformActionTransaction | Promise<PerformActionTransaction> {
      ...
      ["to", "from"].forEach((key) => {
         ...
         if (isPromise(addr)) {
            promises.push((async () => { (<any>request)[key] = '0x' + TRON_ADDRESS_PREFIX + convertToHexAddress(await addr, this.chainNamespace).substring(2); })());
         } else {
