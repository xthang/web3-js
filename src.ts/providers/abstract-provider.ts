...
async function getSubscription(_event: ProviderEvent, provider: AbstractProvider): Promise<Subscription> {
  ...
  } else if (provider.chainNamespace === ChainNamespace.solana) {
    if (typeof _event === 'string' && bs58.decode(_event).length == 64) {
      const hash = _event
      return { type: 'transaction', tag: getTag('tx', { hash }), hash }
    }
  } else if (provider.chainNamespace === ChainNamespace.tron) {
    if (typeof _event === 'string' && _event.length == 64) {
      const hash = _event
      return { type: 'transaction', tag: getTag('tx', { hash }), hash }
    }
  }
    ...
    if (event.address) {
      ...
      const addAddress = (addr: AddressLike) => {
        if (isHexString(addr)) {
          ...
          promises.push(
            (async () => {
              addresses.push(await resolveAddress(addr, provider.chainNamespace, provider))
            })()
          )
        }
      }
...
export type PerformActionRequest =
  ...
  | {
      ...
      blockTag: BlockTag
      transactionEncodedMessage?: string
    }
  | {
      method: 'chainId'
    }
...
export class AbstractProvider implements Provider {
  ...
  constructor(chainNamespace: ChainNamespace, _network?: 'any' | Networkish) {
    ...
    if (_network === 'any') {
      ...
      const network = Network.from(this.chainNamespace, _network)
  ...
  protected async perform<T = any>(req: PerformActionRequest): Promise<T> {
    // Create a tag
  ...
  _getAddress(address: AddressLike): string | Promise<string> {
    return resolveAddress(address, this.chainNamespace, this)
  }
  ...
  _getFilter(filter: Filter | FilterByBlockHash): PerformActionFilter | Promise<PerformActionFilter> {
    ...
    return resolve(<Array<string>>address, fromBlock as string, toBlock as string)
  }
  ...
  async #call(tx: PerformActionTransaction, blockTag: string, attempt: number): Promise<string> {
    ...
    try {
      ...
      if (
        ...
        const txSender = await resolveAddress(transaction.to, this.chainNamespace, this)
  ...
  async broadcastTransaction(signedTx: string): Promise<TransactionResponse> {
    ...
    const tx = Transaction.from(signedTx, this.chainNamespace)
    if (tx.hash !== hash) {
  ...
  async lookupAddress(address: string): Promise<null | string> {
    ...
    try {
      ...
      const ensContract = new Contract(this.chainNamespace, ensAddr, ['function resolver(bytes32) view returns (address)'], this)
      ...
      const resolverContract = new Contract(this.chainNamespace, resolver, ['function name(bytes32) view returns (string)'], this)
