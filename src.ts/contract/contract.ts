  ...
  return await Promise.all(
    inputs.map((param, index) => {
      return param.walkAsync(args[index], (type, value) => {
        ...
        if (type === 'address') {
          return resolveAddress(value, runner.provider.chainNamespace, resolver)
        }
        return value
      })
    })
...
function buildWrappedMethod<A extends Array<any> = Array<any>, R = any, D extends R | ContractTransactionResponse = ContractTransactionResponse>(
  contract: BaseContract,
  key: string
): BaseContractMethod<A, R, D> {
  ...
  const populateTransaction = async function (...args: ContractMethodArgs<A>): Promise<ContractTransaction> {
    ...
    return Object.assign(
      ...
      await resolveProperties({
        ...
        solana: contract.runner.provider.chainNamespace === ChainNamespace.solana ? undefined : undefined,
        tron:
          contract.runner.provider.chainNamespace === ChainNamespace.tron
            ? {
...
function _ContractBase(): new (chainNamespace: ChainNamespace, target: string, abi: InterfaceAbi, runner?: null | ContractRunner) => BaseContract &
