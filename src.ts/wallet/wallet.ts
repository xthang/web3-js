...
import { SolanaWallet, TransactionLike, TransactionResponse, TronProvider, TypedDataDomain, TypedDataField, computeAddress } from '../ethers'
import { AbstractSigner, Provider, SolanaJsonRpcProvider, TransactionRequest } from '../providers/index'
...
import { decryptKeystoreJson, decryptKeystoreJsonSync, encryptKeystoreJson, encryptKeystoreJsonSync, isKeystoreJson } from './json-keystore'
...
export class Wallet extends AbstractSigner implements IWallet {
  ...
  constructor(key: string | SigningKey, provider?: null | Provider) {
    ...
    if (provider) {
      if (provider.chainNamespace === ChainNamespace.eip155) this.#wallet = new Eip155Wallet(key, provider)
      else if (provider.chainNamespace === ChainNamespace.solana) {
        assert(provider instanceof SolanaJsonRpcProvider, 'Solana Wallet can only connect with a Solana provider')
        this.#wallet = new SolanaWallet(key, provider)
  ...
  get wallet(): AbstractSigner & IWallet {
    return this.#wallet
  }
