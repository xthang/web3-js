...
describe('Test CCIP execution', function () {
  ...
  const verify = function (sender: string, data: string, result: string): void {
    const check = concat([toBeArray(dataLength(sender)), sender, toBeArray(dataLength(data)), data])
  ...
  it('testGet should fail with CCIP not explicitly enabled by overrides', async function () {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        const offchainErrorData =
          ''
  ...
  it('testGetFail should fail if all URLs 5xx', async function () {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        return isError(error, 'OFFCHAIN_FAULT') && error.reason === '500_SERVER_ERROR' && JSON.stringify(error.info) === infoJson
      }
    )
  })
  ...
  it('testGetSenderFail should fail if sender does not match', async function () {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        const errorArgsJson =
          '["0x0000000000000000000000000000000000000000",["https://ethers.ricmoo.workers.dev/test-ccip-read/{sender}/{data}"],"0x1234","0xb1494be1","0x4d792065787472612064617461"]'
        const offchainErrorData =
          ''
        return (
          isCallException(error) &&
          error.data === offchainErrorData &&
          error.revert &&
  ...
  it('testGetMissing should fail if early URL 4xx', async function () {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        return isError(error, 'OFFCHAIN_FAULT') && error.reason === '404_MISSING_RESOURCE' && JSON.stringify(error.info || '') === infoJson
      }
    )
  })
