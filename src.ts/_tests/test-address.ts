...
import { convertToHexAddress, getIcapAddress, getCreateAddress, getCreate2Address } from '../index.js'
...
describe('computes checksum address', function () {
  ...
  invalidAddresses.forEach(({ name, value }) => {
    it(`correctly fails on invalid address: ${name}`, function () {
      assert.throws(
        ...
        function (error: any) {
          return error.code === 'INVALID_ARGUMENT' && error.message.match(/^invalid address/) && error.argument === 'address' && error.value === value
        }
      )
    })
  })
  ...
  it('correctly fails on invalid checksum', function () {
    ...
    assert.throws(
      ...
      function (error: any) {
        return error.code === 'INVALID_ARGUMENT' && error.message.match(/^bad address checksum/) && error.argument === 'address' && error.value === value
      }
    )
  })
  ...
  it('correctly fails on invalid IBAN checksum', function () {
    ...
    assert.throws(
      ...
      function (error: any) {
        return error.code === 'INVALID_ARGUMENT' && error.message.match(/^bad icap checksum/) && error.argument === 'address' && error.value === value
      }
    )
  })
})
...
describe('computes create2 address', function () {
  ...
  it('correctly fails on invalid salt', function () {
    ...
    assert.throws(
      ...
      function (error: any) {
        return error.code === 'INVALID_ARGUMENT' && error.argument === 'salt' && error.value === badSalt
      }
    )
  })
  ...
  it('correctly fails on invalid initCodeHash', function () {
    ...
    assert.throws(
      ...
      function (error: any) {
        return error.code === 'INVALID_ARGUMENT' && error.argument === 'initCodeHash' && error.value === badInitCodeHash
      }
    )
  })
})
