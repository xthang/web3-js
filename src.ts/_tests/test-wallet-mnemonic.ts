...
describe('Tests Bad Mnemonics Fail', function () {
  ...
  for (const _phrase of badLengths) {
    ...
    it(`correctly fails on invalid mnemonic length: ${_phrase.length}`, function () {
      ...
      assert.throws(
        ...
        function (error: any) {
          return error.code === 'INVALID_ARGUMENT' && error.message.match(/^invalid mnemonic length/) && error.argument === 'mnemonic' && error.value === '[ REDACTED ]'
        }
      )
    })
  }
  ...
  it('correctly fails on invalid mnemonic word', function () {
    ...
    assert.throws(
      ...
      function (error: any) {
        return error.code === 'INVALID_ARGUMENT' && error.message.match(/^invalid mnemonic word at index 11/) && error.argument === 'mnemonic' && error.value === '[ REDACTED ]'
      }
    )
  })
  ...
  it('correctly fails on invalid mnemonic checksum', function () {
    ...
    assert.throws(
      ...
      function (error: any) {
        return error.code === 'INVALID_ARGUMENT' && error.message.match(/^invalid mnemonic checksum/) && error.argument === 'mnemonic' && error.value === '[ REDACTED ]'
      }
    )
  })
  ...
  for (const _entropy of badEntropyLengths) {
    ...
    it(`correctly fails on invalid entropy length: ${_entropy.length}`, function () {
      assert.throws(
        ...
        function (error: any) {
          return error.code === 'INVALID_ARGUMENT' && error.message.match(/^invalid entropy size/) && error.argument === 'entropy' && error.value === '[ REDACTED ]'
        }
      )
    })
  }
