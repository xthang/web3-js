...
describe('Tests Bad Math Values', function () {
  ...
  for (const { name, value, error, width } of badHex) {
    it(`correctly fails on bad toBeHex values: ${name}`, function () {
      assert.throws(
        ...
        (e: any) => {
          return isError(e, 'NUMERIC_FAULT') && e.fault === 'overflow' && e.message.startsWith(error)
        }
      )
    })
  }
  ...
  it(`correctly fails on nad toBeArray values: negative value`, function () {
    assert.throws(
      ...
      (e: any) => {
        return isError(e, 'NUMERIC_FAULT') && e.fault === 'overflow' && e.message.startsWith('unsigned value cannot be negative')
      }
    )
  })
})
