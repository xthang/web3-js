...
import { concat, dataSlice, id, toBeArray, zeroPadValue, isCallException, isError, Wallet, Eip155Wallet } from '../index.js'
    ...
    for (const method of ['call', 'estimateGas']) {
      for (const providerName of providerNames) {
        ...
        it(`tests panic code: ${providerName}.${method}.${reason}`, async function () {
          ...
          const data = concat([dataSlice(id('testPanic(uint256)'), 0, 4), zeroPadValue(toBeArray(code), 32)])
  ...
  for (const providerName of providerNames) {
    ...
    it(`tests insufficient funds: ${providerName}`, async function () {
      ...
      await assert.rejects(
        ...
        (error) => {
          return isError(error, 'INSUFFICIENT_FUNDS') && typeof error.transaction.from === 'string' && error.transaction.from.toLowerCase() === w.address.toLowerCase()
        }
      )
    })
  }
