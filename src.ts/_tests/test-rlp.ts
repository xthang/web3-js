...
describe('Test bad RLP Data', function () {
  it('correctly fails encoding data with invalid values', function () {
    assert.throws(
      ...
      (error: any) => {
        return error.code === 'INVALID_ARGUMENT' && error.argument === 'object' && error.value === 1234
      }
    )
  })
  ...
  it('correctlyfails decoding data with trailing junk', function () {
    assert.throws(
      ...
      (error: any) => {
        return error.code === 'INVALID_ARGUMENT' && error.message.match(/^unexpected junk after rlp payload/) && error.argument === 'data' && error.value === '0x0042'
      }
    )
  })
  ...
  it('correctlyfails decoding short data', function () {
    assert.throws(
      ...
      (error: any) => {
        return (
          error.code === 'BUFFER_OVERRUN' && error.message.match(/^data too short/) && Buffer.from(error.buffer).toString('hex') === '' && error.offset === 1 && error.length === 0
        )
      }
    )
  })
