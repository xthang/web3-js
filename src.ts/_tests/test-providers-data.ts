...
import { networkFeatureAtBlock, networkNames, testAddress, testBlock, testReceipt, testTransaction } from './blockchain-data'
...
function forEach<T extends { test: string }>(
  prefix: string,
  tests: Record<TestBlockchainNetwork, Array<T>>,
  func: (providerName: string, test: T) => null | ((p: Provider) => Promise<void>)
): void {
      ...
      for (const providerName of providerNames) {
        ...
        retryIt(`${prefix}: ${providerName}:${networkName}.${test.test}`, async function () {
          ...
          try {
            ...
            if ((<any>provider).destroy) {
              ;(<any>provider).destroy()
            }
