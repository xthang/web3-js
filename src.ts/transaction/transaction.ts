...
export class Transaction implements TransactionLike<string> {
  ...
  static from(tx: string | TransactionLike<string> | undefined, chainNamespace: ChainNamespace): Transaction {
    if (tx == null) {
      return new Transaction(chainNamespace)
    }
    ...
    if (typeof tx === 'string') {
      ...
      switch (payload[0]) {
        case 1:
          return Transaction.from(_parseEip2930(payload), chainNamespace)
        case 2:
          return Transaction.from(_parseEip1559(payload), chainNamespace)
      }
    ...
    if (tx.tron) result.tron = tx.tron
    if (tx.solana) result.solana = tx.solana
