  ...
  format(format?: FormatType): string {
    ...
    if (this.isArray()) {
      ...
      if ((this as ParamType).isTuple()) {
        ...
        result += '(' + (this as ParamType).components!.map((comp) => comp.format(format)).join(format === 'full' ? ', ' : ',') + ')'
      } else {
        result += (this as ParamType).type
      }
    }
...
export class EventFragment extends NamedFragment {
  ...
  static from(obj: any): EventFragment {
    ...
    return new EventFragment(_guard, obj.name, obj.inputs ? obj.inputs.map((p: any) => ParamType.from(p, true)) : [], !!obj.anonymous)
  }
...
export class FunctionFragment extends NamedFragment {
  ...
  constructor(
    guard: any,
    name: string,
    stateMutability_: 'payable' | 'nonpayable' | 'view' | 'pure',
